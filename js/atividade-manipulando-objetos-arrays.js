// Atividade: Manipulando Objetos e Arrays.

/* Introdução
Os exercícios a seguir tem como propósito incentivar seu raciocínio lógico e técnico. Aqui você vai desenvolver ainda mais suas habilidades em funções, arrays, condicionais, loops e objetos.
...
function changeDepartmentName(atualName, newName) {
    for (let i = 0; i < departmentList.length; i++) {
      // Verifica se o nome do departamento corresponde ao nome atual
      if (departmentList[i].departmentName === atualName) {
        // Atualiza o nome do departamento com o novo nome
        departmentList[i].departmentName = newName;
  
        // Retorna o objeto do departamento atualizado
        return departmentList[i];
      }
    }
    // Se nenhum departamento com o nome atual for encontrado, retorna "Departamento não encontrado"
    return "Departamento não encontrado.";
  }
  
  console.log(changeDepartmentName('Capitation', 'Capitação'));
...
function giveTheDepartmentABreak(departmentName) {
 ...
 function howManyEmployeesInDepartment(nameDepartment) {
  ...
  // Exemplo de uso: retorna a quantidade de funcionários no departamento com nome "Financial"

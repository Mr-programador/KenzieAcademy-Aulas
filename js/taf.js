...
// Condições para candidatos masculinos
if (gender === "male") {
    // Teste de altura mínima
    if (height >= 1.70) {
        // Teste de repetições na barra ou tempo na barra
        if (barReps >= 6 || barSeconds >= 15) {
            // Teste de abdominais
            if (abs >= 41) {
                // Teste de corrida (pelo menos 3km em 12 minutos ou pelo menos 5km em 20 minutos)
                if ((runDistance >= 3000 && runTime <= 720) || (runDistance >= 5000 && runTime <= 1200)) {
                    // Teste de natação (pelo menos 100 metros em 1 minuto) ou mergulho (no máximo 30 segundos)
    ...
    if (height >= 1.60) {
        ...
        if (barReps >= 5 || barSeconds >= 12) {
            // Teste de abdominais
            if (abs >= 41) {
                // Teste de corrida (pelo menos 4km em 15 minutos ou pelo menos 6km em 22 minutos)
